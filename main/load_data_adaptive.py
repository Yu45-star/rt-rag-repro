...
from tree_decompose_adaptive import adaptive_decompose_and_answer_with_variants
import aiofiles
...
# Output directory uses method_tag instead of the retrieval param string
output_dir = os.path.join(output_dir_root, dataset_name, method_tag)
...
async def write_result_to_file(result, file_path):
    async with aiofiles.open(file_path, 'a', encoding='utf-8') as fout:
        ...
        # 6 adaptive stats fields
        adaptive = result.get("adaptive_stats", {})
        await fout.write(f"adaptive_rewrite_triggered: {int(adaptive.get('rewrite_triggered', 0))}\n")
        await fout.write(f"adaptive_rewrite_effective: {int(adaptive.get('rewrite_effective', 0))}\n")
        await fout.write(f"adaptive_low_confidence_nodes: {int(adaptive.get('low_confidence_nodes', 0))}\n")
...
async def process_example(example, idx, retry_count=0, debug_collector=None, force_write_debug=False):
    ...
    try:
        while True:
            async with semaphore:
                ...
                try:
                    result_tuple = await loop.run_in_executor(
                        None,
                        lambda: adaptive_decompose_and_answer_with_variants(
                            question=question,
                    ...
                    predicted_answer, adaptive_stats = result_tuple
                except Exception as e:
                    predicted_answer = f"Error: {str(e)}"
                    adaptive_stats = {}
                    collector.add_error(
                        "load_data.executor", str(e),
                        {"exception_type": type(e).__name__, "retry_count": current_retry},
                    )
            ...
            collector.add_error(
                "load_data.retry", predicted_answer,
                {"qid": qid, "retry_count": current_retry + 1},
            )
        ...
        collector.add_error(
            "load_data.process_example", str(e),
            {"exception_type": type(e).__name__, "retry_count": current_retry},
        )
        collector.set_retry_count(current_retry)
        elapsed = 0.0 if question_started is None else time.perf_counter() - question_started
        collector.set_timing("question_total_seconds", elapsed)
        collector.set_outcome(status="failure", predicted_answer=None,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run adaptive RT-RAG batch inference")
    parser.add_argument("--limit", type=int, default=None,
                        help="Only process at most N examples")
    parser.add_argument("--start-index", type=int, default=0,
                        help="0-based dataset index to resume from")
    parser.add_argument("--qids", type=str, default="",
                        help="Comma-separated qids to process")
    parser.add_argument("--qid-file", type=str, default="",
                        help="File with one qid per line for targeted reruns")
    parser.add_argument("--always-write-debug", action="store_true",
                        help="Write debug JSON logs for every example")
...
async def main(limit=None, start_index=0, target_qids=None, always_write_debug=False):
    ...
    if limit is not None:
        ...
        print(f"Limiting run to {end_index - start_index} examples from index {start_index}")
    elif start_index > 0:
    ...
    if target_qids:
        ...
        selected_indices = [idx for idx in selected_indices
                            if data[idx].get("_id") in target_qid_set]
        missing_qids = [qid for qid in target_qids
                        if qid not in {data[idx].get("_id") for idx in selected_indices}]
    ...
    tasks = [
        process_example(data[idx], idx,
                        force_write_debug=(always_write_debug or bool(target_qids)))
        for idx in selected_indices
    ]

    results = await tqdm_asyncio.gather(*tasks, desc=f"Processing {dataset_name} (adaptive)")

    success_count = sum(1 for r in results if r.get("success", False))
    fail_count = len(results) - success_count

    print(f"Processing completed. Results saved to {output_file_path}")
    print(f"Total: {len(results)}, Success: {success_count}, Failures: {fail_count}")
